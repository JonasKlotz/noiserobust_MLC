...
class MLPDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.linear4 = nn.Linear(512,n_tgt_vocab)
...
class MLPDecoder2(nn.Module):
    ...
    def __init__(
            self, n_tgt_vocab, n_max_seq_e, n_max_seq_d, n_layers=6, n_head=8, d_k=64, d_v=64,
            d_word_vec=512, d_model=512, d_inner_hid=1024, dropout=0.1,enc_transform='mean'):
        super(MLPDecoder2, self).__init__()
        self.n_max_seq = n_max_seq_e
        self.d_model = d_model
        self.dropout = nn.Dropout(dropout)
        self.enc_transform = enc_transform
...
class GraphDecoder(nn.Module):
    def __init__(
            ...
            enc_vec=True,graph_conv=False,attn_type='softmax', emb=None, freeze_emb=False):
        ...
        if emb is None:
            ...
            self.tgt_word_emb = nn.Embedding.from_pretrained(emb, freeze=freeze_emb)
    ...
    def forward(self, tgt, src_seq, enc_output,return_attns=False, int_preds=False):
        ...
        if self.label_mask is not None:
            dec_slf_attn_mask = self.label_mask.repeat(batch_size,1,1).cuda().bool()
        else:
