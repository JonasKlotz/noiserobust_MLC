...
def test_epoch(model, test_data,opt,data_dict, description):
    ...
    if 'resnet' in opt.encoder:
        all_predictions = torch.zeros(len(test_data.dataset),out_len)
        all_targets = torch.zeros(len(test_data.dataset),out_len)
    else:
    ...
    for batch in tqdm(test_data, mininterval=0.5, desc=description, leave=False):
        ...
        if opt.binary_relevance:
            ...
            pad_batch = False
            if opt.multi_gpu and (batch[0][0].size(0) < opt.batch_size) and ('voc' not in opt.dataset) and ('nuswide' not in opt.dataset): 
                pad_batch = True
