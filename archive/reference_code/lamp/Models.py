...
from lamp.Encoders import MLPEncoder,GraphEncoder,RNNEncoder,MLPEncoder2,MLPEncoder3,MLPEncoder1,RESNETEncoder
from lamp.Decoders import MLPDecoder,RNNDecoder,GraphDecoder,MLPDecoder2
...
class LAMP(nn.Module):
    def __init__(
            ...
            graph_conv=False,attn_type='softmax',int_preds=False, src_emb=None, tgt_emb=None, freeze_emb=False):
        ...
        self.enc_vec = False
        if encoder == 'mlp' or enc_transform != '' or encoder == 'resnet':
            self.enc_vec = True
        ...
        if encoder == 'mlp':
            ...
            self.encoder = MLPEncoder1(
                n_src_vocab, n_max_seq_e, n_layers=n_layers_enc, n_head=n_head,
                d_word_vec=d_word_vec, d_model=d_model,d_k=d_k, d_v=d_v,
                d_inner_hid=d_inner_hid,onehot=onehot, dropout=dropout)
        elif encoder == 'mlp2':
            self.encoder = MLPEncoder2(
                n_src_vocab, n_max_seq_e, n_layers=n_layers_enc, n_head=n_head,
                d_word_vec=d_word_vec, d_model=d_model,d_k=d_k, d_v=d_v,
                d_inner_hid=d_inner_hid,onehot=onehot, dropout=dropout)
        elif encoder == 'mlp3':
            self.encoder = MLPEncoder3(
                n_src_vocab, n_max_seq_e, n_layers=n_layers_enc, n_head=n_head,
                d_word_vec=d_word_vec, d_model=d_model,d_k=d_k, d_v=d_v,
                d_inner_hid=d_inner_hid,onehot=onehot, dropout=dropout)
        elif encoder == 'graph':
        ...
        if decoder == 'rnn_m':
            ...
            self.decoder = GraphDecoder(
                ...
                attn_type=attn_type, emb=tgt_emb, freeze_emb=freeze_emb)
        elif decoder == 'mlp':
        ...
        bias = False
        if self.decoder_type in ['mlp','mlp2','graph','star'] and not proj_share_weight:
            bias = True


        assert d_model == d_word_vec

        if self.decoder_type != 'mlp' and self.decoder_type != 'mlp2':
    ...
    def forward(self, src, adj, tgt_seq, binary_tgt,return_attns=False,int_preds=False):
        ...
        elif self.decoder_type == 'mlp' or self.decoder_type == 'mlp2':
            seq_logit = dec_output
        else:
