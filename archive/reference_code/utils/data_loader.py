...
from utils import coco, nuswide, voc
from os import path
...
def process_data(data,opt):
    ...
    if (opt.adj_matrix_lambda > 0):
        ...
        label_cnt = [0] * (len(data['dict']['tgt'])-4)
        adj_matrix0 = torch.eye(len(data['dict']['tgt'])-4)
        adj_matrix1 = torch.eye(len(data['dict']['tgt'])-4)
        for sample in data['train']['tgt']:
            sample2 = sample
            for i,idx1 in enumerate(sample[1:-1]):
                for idx2 in sample2[i+1:-1]:
                    if idx1 != idx2:
                        adj_matrix0[idx1-4,idx2-4] = 1
                        adj_matrix0[idx2-4,idx1-4] = 1
                        adj_matrix1[idx1-4,idx2-4] += 1
                        adj_matrix1[idx2-4,idx1-4] += 1
                        label_cnt[idx1-4] += 1
                        label_cnt[idx2-4] += 1

        adj_matrix2 = torch.eye(len(data['dict']['tgt'])-4)
        for i in range(adj_matrix2.shape[0]):
            for j in range(adj_matrix2.shape[1]):
                if i == j:
                    pass
                else:
                    adj_matrix2[i,j] = adj_matrix1[i,j]/label_cnt[i]
        #label_adj_matrix = adj_matrix2
    else:
        adj_matrix0,adj_matrix1,adj_matrix2 = None,None,None
    ...
    if opt.encoder == 'graph' or opt.encoder == 'VAE2': 
        vocab = 0
        for sample in data['train']['tgt']: 
            vocab = max(vocab, max(sample))
        toremove = list(range(vocab-3, vocab+1))
        for i in range(len(data['train']['tgt'])): 
            data['train']['tgt'][i] = [x for x in data['train']['tgt'][i] if x not in toremove]
        for i in range(len(data['valid']['tgt'])): 
            data['valid']['tgt'][i] = [x for x in data['valid']['tgt'][i] if x not in toremove]
        for i in range(len(data['test']['tgt'])): 
            data['test']['tgt'][i] = [x for x in data['test']['tgt'][i] if x not in toremove]

    try:
        opt.max_token_seq_len_e = data['settings'].max_seq_len
    except KeyError:
        opt.max_token_seq_len_e = 100002
    opt.max_token_seq_len_d = opt.max_ar_length
    
    if opt.summarize_data:
        utils.summarize_data(data)

    if not 'oldsider' in opt.dataset:
    ...
    if 'resnet' in opt.encoder:
        ...
        train_transform = transforms.Compose([
            MultiScaleCrop(opt.image_size, scales=(1.0, 0.875, 0.75, 0.66, 0.5), max_distort=2),
        ...
        if 'voc' in opt.dataset:
            train_dataset = voc.Voc2007Classification('/mnt/beegfs/bulk/mirror/wz346/ML-GCN/data/voc/', 'trainval', noisy_label=opt.noisy_label, noisy_level=opt.noisy_level)
            val_dataset = voc.Voc2007Classification('/mnt/beegfs/bulk/mirror/wz346/ML-GCN/data/voc/', 'test')
        elif 'nuswide' in opt.dataset:
            train_dataset = nuswide.NUSWIDEClassification('/mnt/beegfs/bulk/mirror/wz346/noisy_mlc/data/nuswide/', 'trainval', noisy_label=opt.noisy_label, noisy_level=opt.noisy_level)
            val_dataset = nuswide.NUSWIDEClassification('/mnt/beegfs/bulk/mirror/wz346/noisy_mlc/data/nuswide/', 'test')
        elif 'coco' in opt.dataset:
            train_dataset = coco.COCO2014('/mnt/beegfs/bulk/mirror/wz346/ML-GCN/data/coco/', 'trainval')
            val_dataset = coco.COCO2014('/mnt/beegfs/bulk/mirror/wz346/ML-GCN/data/coco/', 'test')
        ...
        valid_data = torch.utils.data.DataLoader(val_dataset,
                                                 ...
                                                 num_workers=opt.workers)
        test_data = None
        opt.src_vocab_size = opt.image_size * opt.image_size * 3
    ...
    return train_data,valid_data,test_data,(adj_matrix0,adj_matrix1,adj_matrix2),opt
...
class DataLoader(object):
    ...
    def next(self):
        ...
        if self._iter_count < self._n_batch:
            ...
            tgt_data, tgt_pos = pad_to_longest(tgt_insts,encoder=True)
